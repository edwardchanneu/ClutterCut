      ...
      <div className="max-w-3xl mx-auto w-full flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-red-50/30 text-center shrink-0">
          ...
          <p className="text-sm text-red-600/80">
            Moved <strong className="text-red-700">{response?.movedCount ?? 0}</strong> files, but <strong className="text-red-700">{response?.failedCount ?? 0}</strong> failed.
          </p>
        </div>
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          <ul className="space-y-3">
            {response?.errors?.map((err, idx) => (
              <li key={idx} className="bg-white p-4 rounded-lg border border-red-100 shadow-sm flex flex-col gap-1">
